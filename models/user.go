...
type User struct {
	...
	Provider   AuthProvider      `gorm:"size:16;not null;index:,unique,composite:uni_provider_email" json:"provider"`
	Email      string            `gorm:"size:128;not null;index:,unique,composite:uni_provider_email" json:"email"`
	Identifier string            `gorm:"size:256;not null;index" json:"-"` // hashed password, or provider id
}
